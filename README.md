...
`client/api/proxy.js` is a Vercel Serverless Function that proxies every `/api/*` request to the
...
subsequent request would 401, with no cookie ever actually stored. Two simpler approaches were
tried first and didn't survive contact with production (see `docs/decisions.md` #79/#81): a plain
`vercel.json` rewrite straight to the external Render URL only reliably forwards GET/HEAD, so every
POST came back a `405`; and a `[...path].js` dynamic catch-all function file didn't route correctly
on Vercel at all. `client/vercel.json` now rewrites `/api/(.*)` to the internal path
`/api/proxy?path=$1`, which the plain (non-dynamic) function reads back out. See
...
| 15 | Staff deactivation (Team "remove"), auto-clear exemption for acknowledged orders, and toast notifications on auto-cancel | Done — backend fully verified live| Confirmed a real delete of a staff member with order history is rejected by the database itself (no cascade on `primaryWaiterId`/`waiterId`), which is why this is a deactivation (`isActive`), not a delete. Deactivated Bob live: his existing session died on its next request, a fresh login got a clear "deactivated" message, and he dropped out of the default user list; reactivated him and confirmed login worked again. Tested the last-manager and self-deactivation guards with a temporary admin account. Backdated an acknowledged order and an unacknowledged one both 50 minutes and ran the auto-clear sweep directly only the unacknowledged one was cancelled; a real notification was created for it and delivered through the unread-notifications endpoint. |
